...
export function templateEtecBoost(data: LandingPageData): string {
  ...
  // Bug B (repro TV/arabe 2026-07-28) — `data.benefits` est un champ mort :
  // absent du schema JSON envoyé à DeepSeek depuis le prompt v2.2 (remplacé
  // par `features`), donc toujours `[]` en prod. On lit `data.features`
  // (existe, rempli, déjà dans la langue cible) à la place.
  const features = (data.features ?? []).slice(0, 5)
  const featureCards = features.slice(0, 3)
  ...
  return `<!DOCTYPE html><html lang="${data.language || 'fr'}" dir="${data.language === 'ar' ? 'rtl' : 'ltr'}"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${data.product_name}</title>
...
<p style="font-size:11px;font-weight:700;letter-spacing:0.12em;color:#FF2277;text-transform:uppercase;margin-bottom:12px;">${trans(lang, 'bespoke.badge.bestseller')}</p>
...
${data.social_proof?.rating ? `<div style="display:flex;align-items:center;gap:8px;margin-bottom:20px;"><div style="display:flex;">${'★★★★★'.split('').map(() => `<span style="color:#FF2277;font-size:16px;">★</span>`).join('')}</div><span style="font-size:13px;color:#999;">${data.social_proof.rating}</span></div>` : ''}
...
<ul style="list-style:none;margin-bottom:32px;display:flex;flex-direction:column;gap:10px;">${features.map(f => `<li style="display:flex;align-items:center;gap:10px;"><span style="width:22px;height:22px;border-radius:50%;background:linear-gradient(135deg,#FF2277,#724CE9);color:#fff;font-size:10px;font-weight:800;display:flex;align-items:center;justify-content:center;flex-shrink:0;">✓</span><span style="font-size:14px;color:#555;">${f.title}</span></li>`).join('')}</ul>
<div style="display:flex;gap:12px;"><button class="bo-btn" style="flex:1;text-align:center;">${data.cta || trans(lang, 'cta.orderNow')}</button><button class="bo-btn-alt" style="flex:1;text-align:center;">${trans(lang, 'bespoke.nav.composition')}</button></div>
<div style="display:flex;gap:16px;margin-top:20px;justify-content:center;"><span style="font-size:11px;color:#BBB;display:flex;align-items:center;gap:5px;">${ico.truck(13)} ${trans(lang, 'trust.offeredShort')}</span><span style="font-size:11px;color:#BBB;display:flex;align-items:center;gap:5px;">${ico.lock(13)} ${trans(lang, 'trust.securedShort')}</span><span style="font-size:11px;color:#BBB;display:flex;align-items:center;gap:5px;">${ico.shield(13)} ${trans(lang, 'trust.guaranteedShort')}</span><span style="font-size:11px;color:#039903;font-weight:600;display:flex;align-items:center;gap:5px;">${ico.flash(13)} ${trans(lang, 'trust.inStockShort')}</span></div>
</div></div></section>
${featureCards.length > 0 ? `<section style="padding:80px 24px;background:#FFF0F5;"><div style="max-width:1100px;margin:0 auto;">
...
<h2 style="font-size:32px;font-weight:800;color:#1A1A2E;text-align:center;margin-bottom:56px;">${trans(lang, 'bespoke.section.whyTheyLoveIt')}</h2>
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:24px;" class="bo-grid3">${featureCards.map(f => `<div style="background:#fff;border-radius:14px;padding:36px 28px;text-align:center;"><h3 style="font-size:18px;font-weight:700;color:#1A1A2E;margin-bottom:10px;">${f.title}</h3><p style="font-size:14px;color:#999;line-height:1.7;">${f.description}</p></div>`).join('')}</div></div></section>` : ''}
<section style="padding:80px 24px;background:#fff;"><div style="max-width:1000px;margin:0 auto;">
<h2 style="font-size:32px;font-weight:800;color:#1A1A2E;text-align:center;margin-bottom:48px;">${trans(lang, 'legacy.beforeAfter.title')}</h2>
...
<button style="background:#fff;color:#FF2277;border:none;border-radius:14px;padding:18px 52px;font-family:'Poppins',sans-serif;font-size:15px;font-weight:700;cursor:pointer;">${data.cta || trans(lang, 'cta.orderNow')}</button>
<p style="font-size:12px;color:rgba(255,255,255,0.5);margin-top:20px;">${[trans(lang, 'trust.satisfiedOrRefunded'), trans(lang, 'trust.freeShippingOffered'), trans(lang, 'hero.securePayment')].join(' · ')}</p>
</div></section>
<!-- ═══ SECTIONS DYNAMIQUES (story / social_proof / comparison / testimonials / bonuses / guarantee) ═══ -->
${renderRichSections(data, BOOST_THEME)}
