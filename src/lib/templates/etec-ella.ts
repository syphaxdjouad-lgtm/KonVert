...
<nav style="background:#FDF8FC;border-bottom:1px solid #F0E4F0;padding:14px 24px;"><div style="max-width:1200px;margin:0 auto;display:flex;align-items:center;gap:8px;"><span style="font-size:12px;color:#BBB;">${trans(lang, 'bespoke.nav.home')}</span><span style="font-size:12px;color:#DDD;">›</span><span style="font-size:12px;color:#BBB;">Mode Femme</span><span style="font-size:12px;color:#DDD;">›</span><span style="font-size:12px;color:#2D2D2D;font-weight:600;">${data.product_name}</span></div></nav>
...
<div style="display:flex;flex-direction:column;gap:10px;"><button class="el-btn" style="width:100%;text-align:center;">${data.cta || trans(lang, 'cta.addToCart')}</button><button class="el-btn-soft" style="width:100%;text-align:center;">Ajouter à ma wishlist</button></div>
...
<section style="padding:80px 24px;background:#FDF8FC;"><div style="max-width:1000px;margin:0 auto;"><h2 style="font-family:'Playfair Display',serif;font-size:32px;font-weight:600;color:#2D2D2D;text-align:center;margin-bottom:48px;">${trans(lang, 'bespoke.nav.inspirations')}</h2>
...
<p style="font-size:12px;color:rgba(255,255,255,0.5);margin-top:20px;">${[trans(lang, 'trust.freeShippingOffered'), trans(lang, 'trust.freeReturn'), trans(lang, 'hero.securePayment')].join(' · ')}</p>
</div></section>
<!-- ═══ SECTIONS DYNAMIQUES (story / social_proof / comparison / testimonials / bonuses / guarantee) ═══ -->
${renderRichSections(data, ELLA_THEME)}
