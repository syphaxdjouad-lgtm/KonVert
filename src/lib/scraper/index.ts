...
export async function scrapeProduct(url: string): Promise<ScrapedProduct> {
  ...
  const collectPartial = (p: ScrapedProduct | null) => {
    if (!p) return
    if (!bestPartial) { bestPartial = p; return }
    // On garde le plus complet : score = (title?1:0) + min(images,5) + (price?2:0)
    const score = (x: ScrapedProduct) =>
      (x.title ? 1 : 0) + Math.min(x.images.length, 5) + (x.price ? 2 : 0)
    if (score(p) > score(bestPartial)) bestPartial = p
  }
  ...
  if (apiKey) {
    try {
      ...
      collectPartial(extractPartialFromError(err))
      ...
      if (isEmptyPage) {
        try {
          ...
          collectPartial(extractPartialFromError(err2))
        }
      }
    }
  } else {
  ...
  // un produit "partial" plutôt que de throw. L'UI peut alors afficher un
  // formulaire pré-rempli au lieu d'un mur d'erreur.
  const partial = bestPartial as ScrapedProduct | null
  if (partial && (partial.title || partial.images.length > 0)) {
    console.log('[scraper] ⚠ retour dégradé (partial):', {
      title: partial.title || '(vide)',
...
// Une erreur Firecrawl peut transporter des données partielles via une propriété
// `partial` jointe (cf. scrapeViaFirecrawl). Ça permet de garder le titre/images
// même quand la validation finale rejette (ex : titre = nom de domaine).
function extractPartialFromError(err: unknown): ScrapedProduct | null {
  if (err && typeof err === 'object' && 'partial' in err) {
    return (err as { partial: ScrapedProduct }).partial
  }
  return null
}
...
Return: title, description, price, original_price (if discount visible), images (array of full URLs), rating, reviews_count.`,
    ...
    // de render/extraction. Pour un 1er essai (waitFor=8s) → 22s timeout, pour
    // un retry (waitFor=14s) → 28s timeout. Total max 50s, sous /api/scrape
    // (52s) et /api/generate (60s).
  ...
  // que scrapeProduct puisse l'exposer en mode "partial".
  const partial: ScrapedProduct = {
    ...
    images:         (Array.isArray(item?.images) && item.images.length ? item.images
                      : fromHtml?.images?.length ? fromHtml.images
                      : fromMd?.images || []).slice(0, 8),
