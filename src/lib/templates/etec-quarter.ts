...
<div style="background:#121212;color:rgba(255,255,255,0.7);text-align:center;padding:11px 20px;font-size:12px;font-weight:500;letter-spacing:0.04em;">${data.urgency || trans(lang, 'bespoke.quarter.urgencyFallback')}</div>
...
<div style="display:flex;gap:12px;"><button class="qt-btn" style="flex:1;text-align:center;">${data.cta || trans(lang, 'cta.addToCart')}</button><button class="qt-btn-alt" style="flex:1;text-align:center;">${trans(lang, 'bespoke.nav.details')}</button></div>
<div style="display:flex;gap:24px;margin-top:24px;padding-top:18px;border-top:1.5px solid #E5E5E5;"><span style="font-size:11px;color:#999;display:flex;align-items:center;gap:5px;">${ico.truck(14)} ${trans(lang, 'trust.offeredShort')}</span><span style="font-size:11px;color:#999;display:flex;align-items:center;gap:5px;">${ico.lock(14)} ${trans(lang, 'trust.securedShort')}</span><span style="font-size:11px;color:#999;display:flex;align-items:center;gap:5px;">${ico.return(14)} ${trans(lang, 'trust.returnDays', { n: 30 })}</span></div>
</div></div></section>
...
<p style="font-size:12px;color:rgba(255,255,255,0.3);margin-top:20px;">${[trans(lang, 'trust.freeShippingOffered'), trans(lang, 'trust.freeReturn'), trans(lang, 'hero.securePayment')].join(' · ')}</p>
</div></section>
<!-- ═══ SECTIONS DYNAMIQUES (story / social_proof / comparison / testimonials / bonuses / guarantee) ═══ -->
${renderRichSections(data, QUARTER_THEME)}
