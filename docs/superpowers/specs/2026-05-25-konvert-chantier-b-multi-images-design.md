...
4. Enrichir `renderUniqueMechanismV2` avec une image produit en split layout
...
Scraper → LandingPageData.images: string[] (0-8 URLs filtrées)
         ...
         │
         └──→ renderUniqueMechanismV2(data, theme)
...
| images.length | Hero `<img>` | renderHeroThumbs | renderGallery | renderUniqueMechanismV2 |
|---|---|---|---|---|
...
**Décision sur l'image de `renderUniqueMechanismV2` :**
...
KONVERT_GALLERY=false  # rollback : renderGallery, renderHeroThumbs et l'image
                       # dans renderUniqueMechanismV2 retournent '' / sont skip
```
...
### 4.3 Image dans renderUniqueMechanismV2
...
**`renderUniqueMechanismV2` (régression) :**
...
**Feature flag :**
- `KONVERT_GALLERY=false` → `renderHeroThumbs` retourne `''`, `renderGallery` retourne `''`, `renderUniqueMechanismV2` ne contient pas l'image
...
- `renderUniqueMechanismV2` garde la même signature
...
- Comportement : galerie hero retourne `''`, section gallery skip, `renderUniqueMechanismV2` ignore l'image (= comportement chantier A pur)
...
| Enrichissement `renderUniqueMechanismV2` + tests régression | 45min |
